...
let stArray:any = []; // ARRAY TO STORE STUDENTS CREDIENTIALS
...
function feeSelection(programme: string): number {
...
class Student {
    ...
    constructor(name: string,roll_number:string, dateOfBirth: string, email: string, cellNumber: string,programme:string,total_fee:number,fee_payed:number,balance:number) {
        this.name = name;
    ...
    let studentCredentials = new Student(studentInput.studentName, idGen(), studentInput.studentDateOfBirth,
        studentInput.studentEmai, studentInput.studentCellNumber, studentInput.programmeSelect, feeSelection(studentInput.programmeSelect),
        studentInput.payfee,viewBalance())
  ...
  let viewInput = await inquirer.prompt([ 
   ...
   if(viewInput.confirmation=== "no"){ 
    ...
    process.exit();
     ...
     // VIEW FUNCTION
   function view(val:string){ 
    if(stArray.indexOf(val)== -1){ 
       let target:any = stArray.find((obj:any) => obj.roll_number === val.toUpperCase())
       console.log(target);
       }else{ 
